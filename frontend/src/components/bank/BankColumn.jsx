...
function BankNameCell({ row, onPreview }) {
    ...
    return (
        <div className="flex items-center gap-2">
            <span
                ...
                onClick={(e) => {
                    if (!can("bankView")) return;
                    e.stopPropagation();
                    if (onPreview) onPreview(bank.bankId);
                }}
            >
...
export const getBankColumns = (onPreview, onEdit) => [
    {
        accessorKey: "bankName",
        header: sortableHeader("Bank Name"),
        cell: ({ row }) => <BankNameCell row={row} onPreview={onPreview} />,
            ...
            return (
                <div className="flex items-center gap-2">
                    {can("bankView") && (
                        <button
                            title="View Details"
                            onClick={(e) => { e.stopPropagation(); if (onPreview) onPreview(bank.bankId); }}
                    ...
                    {can("bankUpdate") && (
                        <button
                            title="Edit"
                            onClick={(e) => { e.stopPropagation(); if (onEdit) onEdit(bank.bankId); }}
