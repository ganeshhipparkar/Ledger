...
import QuotationCard, { QuotationStatusBadge } from "./QuotationCard";
import { getQuotationTableColumns } from "./QuotationTableColumns";
...
export default function QuotationList() {
    ...
    return (
        <div className="min-h-screen bg-[#f5f6fa]">
            <Header
                page="quotation-list"
                onAddClick={() => router.push("/add-quotation")}
            ...
            <div className="px-6 py-4 space-y-5">
                ...
                {!loading && !error && activeView === "list" && (
                    <div className="w-full bg-white rounded-2xl border border-gray-200 grid grid-cols-1 gap-5 p-4 mb-12">
                        {data.length === 0 && (
                            <p className="text-center text-gray-400 py-16">No quotations found.</p>
                        )}
                        {data.map((q) => (
                            <QuotationListRow
                                key={q.quotationId}
                ...
                {!loading && !error && activeView === "grid" && (
                    <div>
                        {data.length === 0 && <p className="text-center text-gray-400 py-16">No quotations found.</p>}
                            ...
                            <DropdownMenuTrigger asChild>
                                {(() => {
                                    const primaryAction = [
                                        ...
                                        { show: isLatestVersion && q.status === "CONFIRMED" && can?.("quotationAdd"), label: "Clone Quotation" },
                                    ...
                                    <>
                                        ...
                                        {q.invoicePdfPath && (
                                            <>
                                                ...
                                                >
                                                    <span className="w-full h-full px-4 py-2" title="View PDF">View Pdf</span>
                                                </DropdownMenuItem>
                                                <DropdownMenuItem
                                                    className="cursor-pointer text-sm text-gray-700 hover:bg-gray-100 p-0"
                                                    onClick={async (e) => {
