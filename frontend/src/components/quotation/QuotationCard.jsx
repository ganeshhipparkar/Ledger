...
export const QUOTATION_STATUS_COLORS = {
    ...
    SUBMITTED: "mt-2 inline-block rounded-sm bg-blue-100 px-3 py-1 text-sm text-blue-700 font-medium",
    CONFIRMED: "mt-2 inline-block rounded-sm bg-green-100 px-3 py-1 text-sm text-green-700 font-medium",
    CLOSED: "mt-2 inline-block rounded-sm bg-gray-100 px-3 py-1 text-sm text-gray-600 font-medium",
    CANCELLED: "mt-2 inline-block rounded-sm bg-red-100 px-3 py-1 text-sm text-red-700 font-medium",
    EXPIRED: "mt-2 inline-block rounded-sm bg-orange-100 px-3 py-1 text-sm text-orange-700 font-medium",
};
...
export default function QuotationCard({ quotation: q, onStatusUpdate, can, onCustomerClick, onAddedByClick, onQuotationClick }) {
            ...
            <div className="flex items-start gap-4 mb-4">
                ...
                <div className="flex-1 min-w-0 pr-6">
                    <div
                        ...
                        onClick={() => onQuotationClick?.(q.quotationId)}
                    >
                        {q.quotationCode || "-"}
                    </div>
                            ...
                            <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                    ...
                                    >
                                        {(() => {
                                            const primaryAction = [
                                                { show: q.status === "DRAFT" && can?.("quotationUpdate"), label: "Edit" },
                                                { show: q.status === "SUBMITTED" && can?.("quotationUpdate"), label: "Change Quotation" },
                                                { show: can?.("quotationAdd"), label: "Clone Quotation" }
