...
export default function QuotationLevelAdjSidePanel({
    isOpen,
    onClose,
    type = "discount",
    lines: initialLines = [],
    ...
    const title = isDiscount ? "Quotation Discounts" : "Quotation Extra Charges";
    ...
    const handleSave = () => {
        if (isDiscount && total > maxAmount) {
            toast.error(`Total discount (${Number(total).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 4 })}) cannot exceed the quotation total amount (${Number(maxAmount).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 4 })}).`);
            return;
        }
