...
export default function QuotationSidePanel({ id, onClose }) {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                console.error("[QuotationSidePanel] fetch error:", err);
    ...
    const sections = data ? [
        {
            ...
            rows: [
                ...
                { label: "Issue Date", value: formatDisplayDate(data.issueDate) || "—" },
                { label: "Expiry Date", value: formatDisplayDate(data.expiryDate) || "—" },
                {
                    label: "Quotation Pdf",
        ...
        <div className="px-6 py-4 space-y-5">
            <div>
                <h4 className="text-xs font-semibold uppercase tracking-wide text-gray-400 mb-3 border-b pb-2">
                    Item(s) — Total: {data.quotationItems?.length || 0}
                </h4>
                <div className="space-y-3">
                    {(data.quotationItems || []).map((it, idx) => (
