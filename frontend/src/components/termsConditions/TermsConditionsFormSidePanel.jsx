...
import { termsConditionsFormConfig } from "./configs/termsConditionsForm.config";
import Loader from "../ui/Loader";

const getMySwal = () => withReactContent(Swal);

export default function TermsConditionsFormSidePanel({
    isOpen,
    onClose,
    context = "termsConditions-add",
    id,
    onSuccess,
}) {
    ...
    const fetchDetails = async () => {
        ...
        try {
            ...
            toast.error("Failed to load terms & conditions data.", { position: "top-right" });
        } finally {
            setFetching(false);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        const validation = config.schema.safeParse({
            ...(config.mode === "update" ? { termsConditionsId: Number(Array.isArray(id) ? id[0] : id) } : {}),
            ...formData,
        });
        ...
        try {
            ...
            const res = await fetch("/relayapi", {
                ...
                body: JSON.stringify(
                    config.mode === "update"
                        ? { ...bodyPayload, termsConditionsId: numericId }
                        : bodyPayload
                ),
            });
