...
import { getBankBookColumns } from "./BankBookColumn";
import DetailsSidePanel from "../DetailsSidePanel";
import { bankBookSidePanelConfig } from "./configs/bankBookSidePanel.config";
import BankBookFormSidePanel from "./BankBookFormSidePanel";
import { createPortal } from "react-dom";
import { Plus } from "lucide-react";

export default function BankBookList() {
    ...
    const [formContext, setFormContext] = useState("bank-book-add");
    ...
    const openAdd = () => {
        setEditId(null);
        setFormContext("bank-book-add");
        setFormPanelOpen(true);
    };

    const openEdit = (id) => {
        setEditId(id);
        setFormContext("bank-book-update");
        setFormPanelOpen(true);
        setViewId(null);
    };
        ...
        <div className="fixed inset-0 flex flex-col bg-[#f5f6fa] overflow-hidden">
            ...
            <div className="flex-1 w-full px-4 sm:px-6 lg:px-8 py-4 flex flex-col min-h-0 overflow-hidden">
                ...
                <nav className="mb-4 flex items-center space-x-2 text-sm font-medium text-gray-500">
                    ...
                    <span className="text-gray-800">Bank Books</span>
                </nav>

                <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
                    {loading && (
                        <div className="bg-white rounded-xl border border-gray-200 p-8 flex items-center justify-center">
                            <Loader label="Loading bank books..." />
                        </div>
                    )}
                    ...
                    {!loading && !error && (
                        <DataTable
                            ...
                            data={bankBooks}
                            filterableColumns={[
                                { id: "bankBookName", label: "Bank Book Name", filterKey: "bankBookName" },
                                { id: "bankBookCode", label: "Code", filterKey: "bankBookCode" },
                                { id: "bankName", label: "Bank Name", filterKey: "bankName" },
                                { id: "companyName", label: "Company", filterKey: "companyName" },
                                { id: "beneficiaryName", label: "Beneficiary", filterKey: "beneficiaryName" },
                                { id: "accountNumber", label: "Account No.", filterKey: "accountNumber" },
                                { id: "status", label: "Status", filterKey: "status" },
                            ]}
                            onColumnFilterChange={handleSearch}
                            loading={loading}
                            emptyMessage="No bank books found."
            ...
            {viewId &&
                ...
                createPortal(
                    <DetailsSidePanel
                        config={bankBookSidePanelConfig}
                        id={viewId}
