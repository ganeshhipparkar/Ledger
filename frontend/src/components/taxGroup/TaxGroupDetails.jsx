    ...
    const fetchDetails = async () => {
        ...
        try {
            const res = await fetch("/relayapi", {
                ...
                headers: {
                    ...authHeaders(),
                    endpoint: `tax-group-details/${id}`,
                    module: "tax-group",
                },
            });
            const payload = await res.json();
            const data = payload.encrypted ? decryptResponse(payload.encrypted) : payload;
            setItem(data?.taxId ? data : null);
        } catch (err) {
            console.error(err);
            setItem(null);
        } finally {
                        ...
                        <div className="rounded-2xl bg-white p-5 shadow-sm">
                            <div className="border-b pb-5">
                                ...
                                <p className="text-sm text-gray-400 mt-1">
                                    {item.taxCode || "N/A"} ({item.taxValue !== undefined ? `${item.taxValue}%` : "N/A"})
                                </p>
                            </div>
                        ...
                        <div className="grid gap-6 lg:grid-cols-1">
                            ...
                            <div className="rounded-2xl bg-white p-6 shadow-sm">
                                ...
                                <div className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2">
                                    <div>
                                        <div className="text-sm text-gray-500">Tax Name</div>
                                    ...
                                    <div>
                                        <div className="text-sm text-gray-500">Tax Value</div>
                                        <div className="text-[#101010] font-bold text-[#374151] mt-1">
                                            {item.taxValue !== undefined && item.taxValue !== null ? `${item.taxValue}%` : "-"}
                                        </div>
                                    </div>
